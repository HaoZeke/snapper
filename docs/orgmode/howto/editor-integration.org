...
Settings: format-on-save, max line width, language, extra abbreviations.
Format auto-detects from the note's file extension.

For manual installation (before the plugin is listed in Community Plugins):
1. Download =main.js= and =manifest.json= from [[https://github.com/TurtleTech-ehf/snapper/releases][GitHub Releases]]
2. Copy both files to =.obsidian/plugins/snapper/= in your vault
...
The snapper Word add-in provides a task pane for formatting documents with semantic line breaks.
It uses a WebAssembly build -- no binary installation needed.

** Install from AppSource
:PROPERTIES:
:CUSTOM_ID: word-appsource
:END:
...
:PROPERTIES:
:CUSTOM_ID: word-sideload
:END:

1. Download the add-in ZIP from [[https://github.com/TurtleTech-ehf/snapper/releases][GitHub Releases]]
2. In Word: Insert > My Add-ins > Upload My Add-in
3. Select the =manifest.xml= from the extracted ZIP
